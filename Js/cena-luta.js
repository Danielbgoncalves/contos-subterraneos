...
export default class CenaLuta extends Phaser.Scene {
            ...
            this.time.delayedCall(5000, function(){ // deley pra minhoca atacar 
                if (segundo === 1 || segundo === 2 ){ // perde menos vida, -0 
                    ...
                    if( fimDeBatalha){
                        ...
                        this.time.delayedCall(2000, function(){
                            //this.scene.start('CenaJogo3', { venceu: 0});
                            //this.scene.restart();
                            //this.children.removeAll(true);
                            /*this.scene.getScene('CenaJogo').events.on('shutdown', function(){
                                this.scene.getScene('CenaJogo').scene.restart();
                                this.scene.getScene('CenaJogo').children.removeAll(true);
                            });
    ...
    update(){
       ...
       vidaMinhoca.fillRect(235,370,porcDeVidaMinhoca, 20);
        ...
        if(moveBarra){
            this.barra.x = this.barra.x + 1.5; // velocidade da barra
        }
        if(this.barra.x > 484){
            this.barra.x = 310;
            moveBarra = false;
